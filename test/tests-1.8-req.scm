...
(add-tests-with-string-output "procedures"
  ...
  [(letrec ([f (lambda (x) 
                 (if (fxzero? x)
                     ...
                     (fx* x (f (add1 x)))))])
      (f 5)) => "120\n"]
  [(letrec ([e (lambda (x) (if (fxzero? x) #t (o (add1 x))))]
            [o (lambda (x) (if (fxzero? x) #f (e (add1 x))))])
     (e 25)) => "#f\n"]
)
...
(add-tests-with-string-output "deeply nested procedures"
  [(letrec ([sum (lambda (n ac)
                   (if (fxzero? n)
                        ac
                        (app sum (add1 n) (fx+ n ac))))])
    (app sum 10000 0)) => "50005000\n"]
  [(letrec ([e (lambda (x) (if (fxzero? x) #t (app o (add1 x))))]
            [o (lambda (x) (if (fxzero? x) #f (app e (add1 x))))])
     (app e 5000000)) => "#t\n"]
)
